...
| Вход, кабинет, о платформе (**не** «о платформе» — 20a стал «О турнире») | `features/auth/ui/{auth-dialog,auth-form,reset-request-form,reset-password-form,password-hint}.tsx` (вход/регистрация/сброс, редизайн — спека 0038), `app/reset-password/page.tsx` + `widgets/reset-password/reset-password-screen.tsx` (страница по ссылке из письма 0037), `web/src/middleware.ts` + `entities/user/lib/password.ts`/`shared/lib/session-refresh.ts` (автопродление сессии — дефект, обнаруженный только на 0038, не в макете) + `widgets/session-expired/session-expired-dialog.tsx`, `app/dashboard/page.tsx` (сужен до server-обёртки) + `widgets/dashboard/*` (профиль/безопасность — `features/profile/ui/{edit-profile-dialog,change-password-dialog}.tsx`, свои заявки, свои номинации и ближайший бой — `entities/tournament-live/lib/my-view.ts`, новый `FighterService.GetMyFighter`, ADR 0016), `app/about/page.tsx` + `widgets/tournament-about/*` (заменяет заглушку профилем активного турнира) | нет — API из мини-цикла 0037 (сброс/смена пароля, профиль пользователя, поля `Tournament`) и нового `FighterService.GetMyFighter` (0038, ADR 0016) целиком закрывает потребности экрана |
