...
- [x] T1. `proto/hema/v1/pool.proto` + `bout.proto`:
...
- [x] T2. **migrations** — `pool/migrations/00003_pool_arena.sql`:
...
- [x] T3. **domain** — `pool/domain/domain.go`: убрать `LayoutActive/
...
- [x] T4. **testutil** — `pool/testutil/`: обновить `fake_repo.go` (arena_id,
      seat/unseat, выборки), новый `fake_arena_provider.go`
      (`var _ domain.ArenaProvider = …`).
- [x] T5. **service (red→green)** — `pool/service/service.go`:
...
- [x] T6. **repo + api (red→green)** — `pool/repo/queries/pool.sql` (+`make sqlc`)
...
- [x] T7. **bout/api** — смонтировать `BoutPublicService` рядом с admin; тест:
      public доступен без admin, admin — под RequireAdmin. _(bout домен/сервис/
      репо не трогаем.)_

## Join — трек platform (волна 2)

- [x] T8. **platform адаптер** — `internal/platform/pool_arena_provider.go`
      (`ArenaByID/ArenasByIDs` поверх arena service); `platform.go`
      (`poolDeps.Arenas`).
- [x] T9. **wiring-компиляция** — `go build ./...`; `make test` (server).
- [x] T10. **интеграционный** — partial unique `uq_pools_arena`: две посадки на
      одну арену — вторая падает (NFR-4).

## Web — трек B (волна 1, на моках)

- [x] T11. **entities** — `entities/pool/lib/types.ts` (`PoolStatus`, `status/
      arenaId/arenaName`); `entities/pool/model/get-public-pools.ts` (SSR-фетчер).
- [x] T12. **BFF (red→green)** — роуты `pools/[poolId]/seat`, `…/unseat`,
      `arenas/[id]/pools`, `nominations/[id]/public-pools` (+`*.test.ts`, мок
      grpc, маппинг `connect.Code`→HTTP).
- [x] T13. **features** — `features/pool-seating` (api-хуки + ui секции арены);
      бейдж «готовится к запуску» на пуле в `features/nomination-pools`.
- [x] T14. **routes/widgets** — `app/(admin)/admin/arenas/[id]/page.tsx` (секция
...
- [x] T15. **BFF-гейт FR-10** — `app/api/admin/arenas/[id]/archive/route.ts`:
      перед архивацией проверить занятость арены (`GetPoolsForArena`), при
      занятости — `409`. Тест BFF (AC-10).

## Проверка (волна 3)

- [x] T16. `make test-all` зелёный.
- [x] T17. `pnpm exec tsc --noEmit` (менялись protobuf-моки/типы).
- [x] T18. `go build ./...` + `pnpm build`; миграции проверены в полном
      докеризованном стеке (`docker compose up --build`) (Definition of Done).
- [x] T19. Обновить статусы `spec.md`/`plan.md`/`tasks.md`; строка в
