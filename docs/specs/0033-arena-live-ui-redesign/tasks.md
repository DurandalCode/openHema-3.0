...
- [ ] **T25 (red→green).** `features/arena-timer/ui/TimerControls.tsx` —
...
- [ ] **T29.** `make test-all` зелёный.
- [ ] **T30.** `pnpm exec tsc --noEmit` (менялись protobuf-моки — новый
      `BoutJournalEntry`; правило `web/AGENTS.md`).
- [ ] **T31.** `go build ./...` + `pnpm build`.
- [ ] **T32.** Ручной смоук на `make demo-bouts`: переход в панель посреди
...
- [ ] **T33.** Документация: строка «Страница арены» в `docs/design-sync.md`
