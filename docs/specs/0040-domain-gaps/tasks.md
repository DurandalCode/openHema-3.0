...
- [x] T1. `proto/hema/v1/nomination.proto` — без изменений (гейт — доменная
...
- [x] T5. **migrations** — `migrations/00005_withdrawn_seeds.sql`: таблица
      `stage.withdrawn_seeds` (plan.md, DDL целиком).
- [x] T6. **repo (red→green, testcontainers)** —
...
- [x] T7. **service (red→green)** — `service/service_test.go`:
...
- [x] T8. **testutil** — `testutil/fake_repo.go`: методы новых операций
      порта in-memory.
- [x] T9. **адаптеры** — `stage/occupancy_adapter.go`
...
- [x] T10. **repo (red→green)** — `repo/queries/bout.sql`:
...
- [x] T11. **service (red→green)** — тонкая обёртка сервиса над этими двумя
      репо-методами с тестом на fake-репо.
- [x] T12. **адаптеры** — `bout/occupancy_adapter.go`
...
- [x] T17. **repo + api (red→green)** — `repo/queries/fighter.sql`:
...
- [x] T18. **migrations** — `migrations/00004_program.sql`:
      `tournament.program_days`/`tournament.program_items` (plan.md, DDL
      целиком).
- [x] T19. **domain** — `domain/domain.go`: `ProgramDay`, `ProgramItem`,
      `Tournament.Program`, `UpdateInput.Program`.
- [x] T20. **repo (red→green, testcontainers)** — full-replace `program`
      (пустой↔непустой↔другой набор) в той же транзакции, что уже
      full-replace делает для `contacts` → `repo.go`.
- [x] T21. **service + api (red→green)** — валидация непустого `Text`
...
- [x] T22. **BFF (red→green)** — `app/api/nominations/[id]/route.ts` (DELETE)
      + `*.test.ts` (mock grpc): различение двух `FailedPrecondition` по
      `err.rawMessage` → код ошибки в JSON вместо generic 409.
- [x] T23. **UI** — `features/nomination-management/ui/nominations-screen.tsx`
...
- [x] T24. **entities** — `entities/fighter/lib/types.ts`: `linkedAccountId`,
      `linkedAccountDisplayName`, `mergedIntoId`, `status: 'merged'`
      (+ тест парсера).
- [x] T25. **BFF (red→green)** — `app/api/fighters/find-by-account/route.ts`,
      `app/api/fighters/merge/route.ts` + тесты (mock grpc).
- [x] T26. **UI** — `features/fighter-management/ui/*` (+ тесты): бейдж
...
- [x] T27. **entities** — `entities/tournament/lib/types.ts`: `program`
      (+ тест парсера).
- [x] T28. **UI** — `features/tournament-settings/ui/*` (редактор дней/
...
- [x] T29. **UI** — публичная страница/диалог своей заявки подключает блок
...
- [x] T30. **wiring** — `internal/platform`: конструирование
...
- [x] T31. **интеграционные тесты (testcontainers)** — сквозной сценарий
...
- [x] T32. `make test-all` зелёный; `pnpm exec tsc --noEmit`.
- [x] T33. `go build ./...` + `pnpm build`; обновить статус спеки/плана/
