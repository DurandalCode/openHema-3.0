...
- [x] T17. **`docs/design-sync.md`** — обновить строку «Заявки»: реальные
      ...
      `node`-слота.
- [x] T18. **Смоук.** Браузерного инструмента (chromium-cli/Playwright/MCP)
      в этом окружении не оказалось — визуальный смоук в обеих темах
      (клики по чипам живьём, открытие карточки, вид тостов, светлая/тёмная
      тема) **не пройден** и остаётся на ручную проверку пользователем
      перед мержем. Вместо него пройден смоук всего реального стека через
      `make demo` (43 заявки во всех 5 статусах, 2 переполненные номинации)
      + `make server`/`make web` + curl от лица admin: логин,
      `GET /api/applications/overview` (счётчики статусов совпали с сидом
      1:1), `GET /api/applications/[id]` — история с `actorDisplayName`
      верна для событий и заявителя, и админа (главная новая часть спеки,
      FR-19), `POST /api/applications/[id]/confirm-payment` — состояние и
      история обновились после реальной мутации через весь стек (Go →
      Connect → BFF → сериализация), SSR `GET /admin/applications` (в т.ч.
      с `?nominationId=`) отдаёт 200 без ошибок, `PageHeader` с крошкой
      турнира и структура таблицы присутствуют в разметке. Компонентные
      тесты (T11/T12/T13/T14/T15) покрывают клавиатурную навигацию,
      кольцо фокуса, клик по кнопке не открывающий карточку (AC-16) и
      визуальные состояния — но не заменяют человеческий просмотр в
      браузере.

## Проверка

- [x] T19. `make test-all` зелёный (server + web).
- [x] T20. `pnpm exec tsc --noEmit` без ошибок.
- [x] T21. `go build ./...` + `pnpm build` проходят.
- [x] T22. Обновить статусы `spec.md`/`plan.md`/`tasks.md` и строку 0025 в
