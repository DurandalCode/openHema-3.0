...
- [x] T19. **Админская страница этапов** —
...
- [x] T20. **Публичный экран** — `app/nominations/[id]/page.tsx`: схема в
      read-only виде (FR-26), проверка живого обновления (0014).
- [x] T21. **Демо-сид** — сценарий с двумя ветками (`make demo-bouts`):
      реализован AC-2 («группы → сетка за 1-е место + утешительная», обе
      сформированы автоматическим переходом `BuildStage`, а не ручным
      посевом). AC-14 («группы → двойная сетка групп») в демо-сид
      сознательно НЕ включён — вторая полностью доигранная номинация
      потребовала бы либо повторно использовать одну из трёх showcase-арен
      (ломает их существующий сценарий 0011/0013/0014: «идёт»/«готовится к
      запуску»), либо новую арену + новую номинацию с самостоятельно
      доведёнными до конца боями — заметный объём кода ради демо-скрипта,
      когда сам путь groups-формирования уже покрыт
      `TestBuildStage_T11_GroupsHappyPath_DoubleGroups` (T11, сервисный
      юнит-тест на этот же сценарий). Проверено на реальном стеке:
      `make demo-bouts` дважды подряд (идемпотентность) + `make demo` +
      `make demo-registered` не сломаны.
- [x] T22. `make test-all` зелёный + `make test-integration`.
- [x] T23. `go build ./...`, `pnpm exec tsc --noEmit`, `pnpm build`.
- [x] T24. Регресс на реальном стеке: без полного `docker compose down -v`
      (осознанное решение при согласовании — см. ниже) — вместо этого
      `make demo-bouts` дважды подряд на реальном docker-compose Postgres
      (реальные миграции, реальный сервер, весь путь 0019 включая двойной
      плейофф, AC-2) + testcontainers-интеграционные тесты (T13),
      поднимающие чистый Postgres и гоняющие все миграции, включая 00003, с
      нуля при каждом запуске. Сценарий 0018 (AC-18) не задет — регресс
      покрыт `bracket_test.go`/`TestCreateStage_E2E_HappyPath` в общем
      прогоне тестов (T22). Статусы спеки/плана/задач и строка 0019 в
      `docs/specs/README.md` обновлены; пометка у строки 0018 —
