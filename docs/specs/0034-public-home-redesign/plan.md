...
- `service/tournament_live.go` (новый файл, чтобы не раздувать
  ...
  - `TournamentLive(ctx, tournamentID) (domain.TournamentSnapshot, error)`:
    ...
    4. `ActiveArenas` → карточки площадок: сопоставление по `Pool.ArenaID`.
       Состояние определяется через уже существующий
       `effectiveCurrentBoutID(pool, bouts)` (используется
       `GetPoolsForArena`/`GetBoutBoard`, не новый код): его результат —
       первый непроведённый бой пула, `""` — все завершены. `BOUT_IN_PROGRESS` —
       эффективный бой в `in_progress`; `PREPARING` — эффективный бой
       `not_started` (пул стоит, ни один бой не начат — ровно FR-14); `FREE`
       — пула на арене нет **или** пул стоит, но эффективного боя нет (все
       бои завершены, пул ещё не снят администратором явным `UnseatPool`).
       Второй случай FREE — сознательное сужение против макета (тот
       показывает под «свободна» ещё и итог только что отыгранного пула,
       «пул D завершён · итог: Дорохов, Ясенев») — это отдельные доменные
       данные (протокол пула) вне FR-14, в карточке в этом инкременте не
       показываются: `nomination_name`/`pool_name`/`current_bout` у такой
       карточки пусты, как и у пустой арены;
