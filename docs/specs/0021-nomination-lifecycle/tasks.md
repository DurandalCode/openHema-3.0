...
- [ ] **T21. интеграционные (`nomination`)** — миграция 00003 применяется,
      `execution_state` читается/пишется, публичный `GetNomination` отдаёт
      `ACTIVE` после push'а.
- [ ] **T22. интеграционные (`stage`)** — сквозной сценарий: группы →
      доиграть → сформировать сетку → доиграть → номинация `FINISHED` +
      протокол; пересмотр результата возвращает `ACTIVE` (AC-3/AC-4).
- [ ] **T23.** `make test-all` зелёный; `make test-integration` (Docker).
- [ ] **T24.** `pnpm exec tsc --noEmit`; `go build ./...` + `pnpm build`.
- [ ] **T25.** Обновить статус спеки/плана/задач и строку в
