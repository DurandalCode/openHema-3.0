...
- `repo/repo.go` + `repo/queries/application.sql` — `ListByTournament`
  ...
  `ILIKE` по имени/клубу (кейс-инсенситив), `LIMIT`/`OFFSET`, плюс отдельный
  `-- name: CountByTournamentStatus :many` (`GROUP BY status`) для
  status_counts. Имя/клуб для `ILIKE` — уже читаемые поля read-модели
  заявки (используются сегодня в `select-applications.ts` тем же способом).
