...
- [x] T1. `proto/hema/v1/common.proto` (`User.club`),
...
- [x] T2. **pkg/mail (red→green)** — `server/pkg/mail/mail_test.go`: лог-адаптер
      ...
      `logger.go`, `smtp.go`.
- [x] T3. **domain** — `modules/auth/domain/domain.go`: `User.Club`,
...
- [x] T4. **testutil** — `modules/auth/testutil/fake_repo.go`: новые методы
      порта in-memory + `FakeMailer` (запоминает последнее письмо);
      `var _ domain.Repository = (*FakeRepo)(nil)`.
- [x] T5. **service: политика пароля (red→green)** —
...
- [x] T6. **service: запрос сброса (red→green)** —
...
- [x] T7. **service: установка пароля по ссылке (red→green)** — тесты: успех
...
- [x] T8. **service: смена пароля, профиль, обрыв сессий (red→green)** —
...
- [x] T10. **domain** — `modules/tournament/domain/domain.go`: 6 новых полей
      в `Tournament` и в структуре обновления (`EntryFeeMinor *int64`).
- [x] T11. **service (red→green)** — `service/service_test.go`: регламент —
...
- [x] T15. **BFF: пароль и профиль (red→green)** —
...
- [x] T16. **типы и сериализация (red→green)** — тесты `userToJson`
...
- [x] T17. **форма профиля турнира (red→green)** —
...
- [x] T23. **сквозной сценарий сброса** — интеграционный тест на лог-адаптере:
      запрос → токен из письма → установка пароля → вход новым паролем →
      старый refresh отклонён (AC-3, AC-11, AC-17).
- [x] T24. **проверка** — `make test-all` зелёный;
