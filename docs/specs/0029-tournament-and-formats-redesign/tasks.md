...
- [x] J1. **`docs/design-sync.md`** — обновить строку «Турнир и форматы»
...
- [x] J2. `make test-web` зелёный; `pnpm exec tsc --noEmit` чистый. (196
      test files / 1482 tests зелёных после мержа обоих треков.)
- [x] J3. `pnpm build`; `make test` + `go build ./...` — подтвердить, что
      сервер не задет. Все три зелёные, `go test` полностью из кэша
      (server не менялся).
- [x] J4. **Ручной смоук** на `make dev` через реальный headless-браузер
      (Playwright): вход admin, правка профиля турнира с живым превью
      (название/описание/даты/контакты), полоса «Несохранённые изменения:
      …» и «Отменить правки» (возврат без запроса к серверу), пустое
      название блокируется инлайн-ошибкой «Введите название турнира» и
      превью синхронно показывает заглушку «Турнир скоро появится»,
      сохранение даёт тост «Профиль турнира сохранён»; экран форматов —
      шапка со счётчиком, карточка со сводкой схемы и «обновлён сегодня»,
      переименование в модалке (пустое имя блокируется, модалка не
      закрывается), удаление — `ConfirmDialog` с точным перечислением
      последствий и без поля ввода имени, после удаления — пустое
      состояние с переходом «К номинациям». Скриншоты по каждому шагу,
      `console --errors` пуст. Тестовые данные (пресет, правка турнира)
      восстановлены до сида после прогона.
- [x] J5. Обновить статусы `spec.md`/`plan.md`/`tasks.md` на `done` и строку
