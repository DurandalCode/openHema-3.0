...
- [x] T11. **действия этапа (red→green)** —
...
- [x] T13. **Границы слоёв (AC-16)** — грепом убедиться, что ни один файл
      `features/**` не импортирует другую фичу, а композиция страницы живёт
      в `widgets/stage-page/**`.
- [x] T14. `make test-web` зелёный (в т.ч. переписанные тесты 0030/0018).
- [x] T15. `pnpm exec tsc --noEmit` — обязателен: `executionStatus` меняет
      форму protobuf-моков во всех тестах со `Stage` (T1).
- [x] T16. `pnpm build` + ручной смоук на реальном BFF: групповой этап и
      этап-сетка, формирование по правилу, сброс, undo, переход по рельсу
      между этапами, обе темы, ширина < 1024px (FR-11).
- [x] T17. Обновить статус спеки/плана, строку в `docs/specs/README.md` и
