...
<Опционально — заполнять, когда в фиче есть ≥2 куска работы, не имеющих общих
файлов (напр. новый модуль + правки другого модуля + web-слой, каждый со
своими фейками/моками, не ждущий соседа). Если явных дизъюнктных кусков нет —
раздел не нужен, весь чеклист выполняется последовательно как обычно.

Критерий трека — **дизъюнктность файлов**, не просто «логическая
независимость»: если два куска трогают один и тот же файл — это один трек.
Треки одной волны выполняются параллельно (отдельные агенты, изолированные
`git worktree`, см. `.claude/skills/tdd-cycle/SKILL.md`); задачи, которым
нужны файлы нескольких треков (wiring, интеграционные тесты, verification) —
отдельная join-волна после мержа всех треков предыдущей волны в рабочую
ветку.>

| Волна | Трек | Задачи | Файлы (не пересекаются внутри волны) | Зависит от |
| ----- | ---- | ------ | ------------------------------------- | ---------- |
| 0     | —    | T1     | контракты (proto/sql)                 | —          |
| 1     | A    | ...    | ...                                    | волна 0    |
| 1     | B    | ...    | ...                                    | волна 0    |
| 2     | join | ...    | (общие файлы треков A/B)              | треки A и B смержены |
