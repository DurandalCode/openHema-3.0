...
- Слои:
  ...
  - `migrations/` — goose: **проговариваются на уровне DDL целиком, а не
    размыто.** Перечислить: схему, таблицы с колонками и типами, констрейнты
    (CHECK/UNIQUE/FK, в т.ч. partial unique), индексы, сиды. Для каждой
    таблицы — коротко Зачем она (одна таблица на.aggregate root? jsonb-поле?
    дочерняя таблица?). В спеке принятые решения модели фиксируются, здесь —
    их DDL-воплощение. Размытые формулировки вроде «добавить таблицу турнира»
    недопустимы: они прячут структурные решения (напр. вынос контактов в
    отдельную таблицу против jsonb), которые дорого переигрывать на этапе
    реализации. См. ADR 0008.
