...
- [x] T23. **Интеграционные (`integration/`, testcontainers)** — новый файл
      `format_constructor_integration_test.go`. Миграция `00004` вверх
      применяется в каждом тесте через `setup()` (goose, как остальные
      модули); откат вручную проверен ранее на локальной БД (`make migrate`/
      `make migrate-down`) — testdb-хелпер (`internal/testdb`) не даёт
      per-test доступа к `goose down`, инфраструктуру не расширяли ради
      этого. Уникальность имени пресета без учёта регистра/пробелов —
      через реальный `SaveFormatPreset` (`AlreadyExists`). `ReplaceSchema`:
      каскад — старые этапы и их (пустые) пулы физически удалены (прямой
      SQL после `ApplyFormat`, не только «не видны через API»), проверка
      «нет осиротевших пулов»; восстановленный `source_stage_id` — сравнение
      с РЕАЛЬНЫМ id нового этапа (не индексом, не id донора/старой схемы);
      новая сетка получает два контейнера первого круга, как
      `service.CreateStage`. Атомарность на середине транзакции отдельно не
      форсировалась — нет точки инъекции сбоя через RPC-поверхность без
      правки прод-кода ради теста; транзакционность гарантируется тем, что
      `ReplaceSchema` — одна `BeginTx`/`Commit` (repo.go), как и остальные
      многошаговые мутации модуля (`ApplyStageBuild`/`ResetLayout`). Полный
      путь «групповой этап + двойной плейофф (AC-2 спеки 0019) → пресет →
      применение к другой номинации → набор группы, бои, `BuildStage` одной
      из веток по 0019» — через реальный Connect, посев проверен прямым SQL.
- [x] T24. **Ручная проверка** — поднят полный стек (`make migrate` +
      `make demo-bouts` + реальные `server`/`web` процессы) и пройден флоу
      через настоящий HTTP (BFF) на уже готовом демо-сценарии «группы →
      двойной плейофф» (`cmd/demo-bouts` сам засевает его, AC-2 спеки 0019):
      сохранён пресет из готовой номинации (индексы источников/конфиг
      корректны), применение к номинации с составом отклонено 409 с понятным
      текстом (гейт FR-13/FR-14), применение к свежей пустой номинации
      заменило схему целиком с реально резолвленными `sourceStageId` (FR-13),
      переименование/удаление пресета и повторное удаление → 404, удаление
      пресета не изменило уже применённую схему (FR-16), публичный экран
      номинации (0011/0014/0015 — живая сетка, реальные бои) отдаёт 200 и не
      содержит ни слова про пресеты/диагностику/admin-действия (FR-19).
      **Оговорка**: в этом окружении нет браузерного драйвера (chromium-cli/
      Playwright не установлены) — проверка прошла на уровне полного стека
      (Next.js BFF → gRPC → PostgreSQL) через `curl`, а не визуально в
      браузере; сами admin-диалоги (`EditStageDialog`/`SavePresetDialog`/
      `ApplyFormatDialog`/`PresetLibrary`) не сфотографированы, их логика
      покрыта юнит/компонентными тестами (T18–T20).
- [x] T25. **Проверка сборки** — `make test-all` зелёный (server + web);
      `pnpm exec tsc --noEmit` чисто; `go build ./...` и `pnpm build` (Next.js
      прод-сборка, включая новые `/admin/formats`, `/api/formats`,
      `/api/formats/[presetId]`, `/api/nominations/[id]/format`) — оба без
      ошибок.
