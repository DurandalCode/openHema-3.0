...
- Статус: done
- Дата: 2026-07-13
- Инкремент: 2026-07-14 — undo для «Сбросить раскладку» (третий класс undo).
  Правки: обзор (undo трёх классов), миграция 00002 (CHECK + `'reset'`), DDL
  комментарий, семантика undo (`UndoReset` + JSONB shape), риски.
- Спека: `./spec.md`
...
(авто/удаление пула/сброс раскладки) — снапшот в JSONB на `pool_layout`
(решение №16).
...
CREATE TABLE pool.pool_layouts (
    ...
    -- 'reset' → {"pools":[{"number":N,"fighter_ids":[...]},...]} (восстановить
    -- все пулы + членства после сброса раскладки).
    ...
    CONSTRAINT chk_layouts_undo   CHECK (undo_kind IN ('','auto','delete_pool','reset'))
);
...
`modules/pool/migrations/00002_undo_reset.sql` (goose Up/Down, инкремент
2026-07-14 — undo для сброса раскладки):
...
-- Существующие строки undo_data корректны: никто ещё не писал 'reset' (его
-- не было в CHECK). Новой колонки нет — 'reset' использует существующую
-- undo_data JSONB с новой формой {"pools":[...]}.
...
- Каждое mutating-действие **сбрасывает** `undo_kind=''`, **кроме**:
  - `AutoDistribute` → пишет `undo_kind='auto'`, `undo_data.fighter_ids` = кого
    расставило (diff нераспределённых до/после);
  - `DeletePool` → `undo_kind='delete_pool'`, снапшот `{number, fighter_ids}`;
  - `ResetLayout` → `undo_kind='reset'`, снапшот `{pools:[{number, fighter_ids}, ...]}`
    (все пулы номинации с их членствами на момент сброса).
- `Undo`:
  ...
  - `reset` → апсертом пересоздать **все** пулы из снапшота с теми же `number`
    + членства (номера свободны, т.к. любая мутация обнулила бы undo —
    AC-13a4). Восстановление идемпотентно: повторный undo даёт тот же результат
    (пулы уже существуют с теми же номерами и членствами).
...
- **Идемпотентность undo `delete_pool`.** Восстановление номера опирается на
  ...
  AC-13a2/13a3.
- **Идемпотентность undo `reset`.** Восстановление номеров всех пулов из
  снапшота опирается на тот же инвариант «любая мутация обнуляет undo» → все
  номера свободны. Проверить в тесте AC-13a4 (восстановление после сброса) и
  AC-13b (сброс обнуляет предыдущий undo, но создаёт свой). Повторный undo
  идемпотентен: пулы уже существуют с теми же номерами и членствами —
  апсерты/insert-where-not-exists дают тот же результат.
