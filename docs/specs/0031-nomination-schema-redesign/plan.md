...
| `nomination-schema.tsx` (**удаляется**) | Заменяется списком выше. Ветка `mode="public"` (0019 FR-26) сегодня не рендерится ни одним потребителем — публичную проекцию схемы авторит спека 0035, когда до неё дойдёт очередь |
| `nomination-schema.test.tsx` (**удаляется**) | Покрытие переезжает в тесты новых файлов |
