...
- [x] T29. `make test-all` зелёный; `pnpm exec tsc --noEmit`;
      `go build ./...` + `pnpm build`.
- [x] T30. **Регресс на реальном стеке** — `docker compose up --build`
      ...
      Прогнано против живого докер-стека через прямые запросы к BFF
      (эквивалент кликов — те же ручки, что и UI): создана и оспорена ничья
      (409 `ErrDrawNotAllowed`), досыгран круг с продвижением победителя и
      баем, пересмотр результата снял продвижение (AC-8), запущенный
      следующий бой заблокировал пересмотр (409 `ErrDownstreamStarted`,
      AC-9), обе половины поставлены на разные площадки одновременно и
      обе отдают корректную доску (AC-5a/AC-5), публичный SSR-снапшот
      отдаёт группы и сетку вместе (AC-12), групповой пул продолжает
      принимать счёт как раньше (AC-17). Все проверки зелёные.
- [x] T31. Обновить статус спеки/плана/`tasks.md` и строку в
