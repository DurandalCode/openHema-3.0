...
- [x] T42. `make test-all` зелёный.
- [x] T43. `pnpm exec tsc --noEmit` (менялись protobuf-типы и моки).
- [x] T44. `go build ./...` + `pnpm build`.
- [x] T45. `docker compose up --build`: миграции обоих модулей проходят в
      полном стеке, том хранилища переживает пересборку (FR-37) — оба
      файла на месте после `--build`, письма без SMTP уходят в журнал
      (NFR-3).
- [x] T46. Ручная проверка сквозных сценариев: регистрация → письмо →
...
- [x] T47. Обновить статусы `spec.md`/`plan.md`/`tasks.md` и строку `0042`
