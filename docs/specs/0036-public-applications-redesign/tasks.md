...
- [x] T13. `make test-web` зелёный (весь web, включая существующие тесты
      номинации/главной, задетые пропом `isAuthenticated`). Итог после
      join-волны: 271 файл / 2019 тестов, без регрессий.
- [x] T14. `pnpm exec tsc --noEmit`. Чисто.
- [x] T15. `pnpm build`. Прод-сборка прошла, новый роут
      `/nominations/[id]/apply` появился в манифесте.
- [x] T16. **Ручной смоук** на реальном BFF + сервере + Postgres
      (`make demo`, headless-браузера в среде реализации нет — как в
      0026/0027/0030–0035): подтверждено curl'ом на реальных мутациях —
      подача заявки с клубом/экипировкой (AC-3), повторная подача → 409
      «Вы уже подали заявку в эту номинацию» (AC-6), подача в номинацию с
      закрытым приёмом (закрыт админским `close-registration` по ходу
      смоука) → 409 «Приём заявок в эту номинацию завершён» (AC-5, обе
      ветки — BFF и SSR-страница `/apply`, независимая от клиентского
      запроса), 404 «Номинация не найдена» на несуществующей номинации,
      отметка оплаты (`APPLICATION_STATE_SUBMITTED` →
      `AWAITING_PAYMENT_CONFIRMATION`), отзыв (→ `WITHDRAWN`) и повторный
      отзыв терминальной заявки → 409, гейтинг гостя на странице номинации
      (FR-14 — CTA отсутствует без сессии), кнопка-ссылка на `/apply` на
      главной для открытых номинаций и подпись «Приём заявок завершён» для
      закрытой (FR-12/FR-13). Визуальная проверка на 1440/390 и клик по
      `ConfirmDialog` отзыва — не выполнялись (нет headless-браузера);
      функциональная корректность диалога и гейтинга действий по состоянию
      покрыта компонентными тестами трека B (`application-card.test.tsx`).
- [x] T17. **Документация** — статусы `spec.md`/`plan.md`/`tasks.md` → done,
