...
- Новых маршрутов нет: список — существующий `GET /api/applications`,
  действия — `POST /api/applications/[id]/{declare-payment,withdraw}`. Обе
  ручки действий получают ту же обработку через общий
  `app/api/applications/[id]/action-error.ts` (найдено на код-ревью после
  первой реализации: `applicationErrorMessage` на клиенте считает любой 409
  уже человеческим текстом, а `errorResponse` без разбора кода отдаёт на
  `FailedPrecondition`/`Aborted` сырой текст Go-домена) —
  `FailedPrecondition` → 409 со своим текстом на действие («Заявку нельзя
  отозвать в текущем состоянии» / «Оплату можно отметить только для
  поданной заявки»), `Aborted` (конфликт версии, ADR 0011) → 409 «Заявка уже
  изменилась — обновите страницу и попробуйте снова», остальное —
  `errorResponse` как раньше.
