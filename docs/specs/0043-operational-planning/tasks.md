...
- [x] T19. **публичная главная (red→green)** — тесты `widgets/home/*`:
...
- [x] T20. **страница номинации и кабинет (red→green)** — тесты
...
- [x] T21. **доска площадок (red→green)** — тесты
...
- [x] T22. `make test-all` зелёный. — весь сервер (372/372 Go-пакетов с
      тестами) и весь веб (372 файла, 2914 тестов) — без исключений.
- [x] T23. `pnpm exec tsc --noEmit` (менялись protobuf-типы — обязательно).
      — чисто.
- [x] T24. `go build ./...` + `pnpm build`. — оба чисто; прод-сборка веба
      включает `/admin/console`, `/api/tournaments/[id]/console` и
      `.../console/stream`.
- [x] T25. `make migrate` на чистой БД: единственная новая миграция
      (`arena/00003_last_freed_at.sql`) применяется и откатывается. —
      проверено на свежем контейнере `docker compose up -d --wait
      postgres`: колонка `last_freed_at` появляется на up, пропадает на
      down, снова появляется на повторном up; остальные 8 модулей — без
      миграций (`no migrations to run`); контейнер остановлен и удалён
      после проверки.
- [x] T26. Обновить статусы `spec.md`/`plan.md`/`tasks.md` на `done` и
