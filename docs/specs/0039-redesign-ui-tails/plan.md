...
| `features/*/api/requests.ts`, `features/*/api/use-*.ts` | Перевод на `apiFetch` — 21 файл (перечень в `tasks.md`). Исключения: `features/auth/api/requests.ts` (FR-19 — там 401 значит «неверные учётные данные») и три публичных живых хука `tournament-live`, `nomination-live`, `arena-live` (FR-20) |
