...
3. колонки: по заголовку (`имя`/`name`, `фамилия`/`surname`/`last_name`,
   `клуб`/`club`, `номинации`/`nominations`, регистр и окружающие пробелы
   не важны); отсутствие колонки имени → `ErrMissingNameColumn`. Если
   фамилия в строке заполнена, `ImportRow.Name` собирается из фамилии и
   имени через пробел; если фамилия пуста, имя используется как полное.
   Пустое имя остаётся пустым даже при заполненной фамилии (FR-6a).
   Порядок колонок в файле произволен, лишние колонки игнорируются;
   **настраиваемого маппинга нет** (вне скоупа);
