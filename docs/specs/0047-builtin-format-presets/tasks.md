...
- [x] T1. `proto/hema/v1/stage.proto` — `RestoreBuiltinPresets` в
...
- [x] T2. **domain: каталог (red→green)** — `domain/catalogue_test.go`:
...
- [x] T3. **domain: порт** — `domain/domain.go`: `SeededPresetKeys` и
      ...
      шаге.
- [x] T4. **testutil** — `modules/stage/testutil/fake_repo.go`: журнал
      заведённых ключей в памяти + две новые операции порта (`var _
      domain.Repository = (*FakeRepo)(nil)` должен снова компилироваться).
- [x] T5. **service (red→green)** — `service/catalogue_test.go` на fake-репо:
...
- [x] T6. **service: применимость каталога (red→green)** — дополнение
      ...
      _(FR-11; AC-2, AC-3.)_
- [x] T7. **repo + migrations** — `migrations/00006_builtin_presets.sql`
...
- [x] T8. **api + wiring (red→green)** — `api/handler_test.go` (httptest +
...
- [x] T9. **BFF (red→green)** — `app/api/formats/restore/route.test.ts`: 401
      ...
      `formatPresetsToJson`).
- [x] T10. **feature (red→green)** — `features/format-presets/api/requests.test.ts`
...
- [x] T11. **ui (red→green)** — `features/format-presets/ui/preset-library.test.tsx`:
...
- [x] T12. `make test-all` зелёный (server + web).
- [x] T13. `pnpm exec tsc --noEmit` (контракт менялся — protobuf-типы web
      перегенерированы).
- [x] T14. `go build ./...` + `pnpm build` — зелёные. Проверка на чистой БД
      сделана тестом `internal/platform/integration/builtin_presets_bootstrap_integration_test.go`
      (composition root + `BootstrapPresets` на свежей testcontainers-БД →
      `ListFormatPresets` отдаёт 10, AC-1) вместо `docker compose down -v &&
      make dev` — тот же композит, что реальный старт сервера, без стирания
      локального дев-окружения разработчика. AC-2/AC-6 (применение целевой
      схемы / удалённый не возвращается) покрыты юнит- и интеграционными
      тестами T5/T6/T7.
- [x] T15. Обновить статус спеки/плана/тасков и строку в индексе
