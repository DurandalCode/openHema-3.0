...
> **Итог реализации.** Первый проход `RestoreBuiltinPresets` (реализован по
> плану выше — фильтр по конфликту имени через `ErrPresetNameTaken`, без
> отдельной проверки принадлежности каталогу) не проходил AC-10: переименование
> встроенного пресета освобождает его исходное имя каталога, и восстановление
> заводило рядом дубликат содержимого под старым именем — обнаружено при
> ревью T6 второго трека. Исправление: журнал получил `preset_id` (FK на
> `stage.format_presets`, `ON DELETE SET NULL`) вместо голого факта «ключ
> заводился». `SeededPresetKeys` (весь журнал, для `SeedBuiltinPresets`,
> FR-7) и `LiveSeededPresetKeys` (только записи с непустым `preset_id`, для
> `RestoreBuiltinPresets`, FR-10) — два разных фильтра поверх одной таблицы:
> переименование id не трогает (запись остаётся «живой», восстановление её не
> касается), удаление обнуляет ссылку (запись «умирает», восстановление
> заводит заново). Миграция 00006 правилась на месте (не новой 00007) — не
> была нигде развёрнута к моменту исправления.
