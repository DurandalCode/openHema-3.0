...
- [x] T1. **entities (red→green)** — `entities/nomination-live/lib/position.test.ts`:
...
- [x] T2. **entities (red→green)** — `entities/stage/lib/schema-chain.test.ts`:
...
- [x] T3. **widget (red→green)** — `widgets/nomination-public/nomination-header.test.tsx`:
...
- [x] T4. **widget (red→green)** — `widgets/nomination-public/schema-chain.test.tsx`
...
- [x] T9. **widget (red→green)** — дополнить `widgets/bracket-view/bracket-view.test.tsx`:
...
- [x] T15. **удаление** — `widgets/nomination-schema/nomination-schema.tsx`
...
- [x] T16. `make test-all` зелёный; `pnpm exec tsc --noEmit`.
- [x] T17. `pnpm build` (web) — сборка страницы без ошибок; глазами
      проверить обе темы и ширину 390px (NFR-3).
- [x] T18. Обновить статус `spec.md`/`plan.md`/`tasks.md`, строку 0035 в
