...
3. **Интеграционные e2e с БД** (build-tag `integration`, `make test-integration`,
   требует Docker) — реальный PostgreSQL через `testcontainers-go` для миграций,
   sqlc-запросов, сквозного Connect-пути (`httptest` server × real PG). См.
   **ADR 0010**. Не входит в `go test ./...` (локальный цикл Docker-free);
   обязателен для модуля, владеющего PG-схемой; гоняется в CI.

   Локация: `server/modules/<name>/integration/*_integration_test.go` с
   `//go:build integration`. БД-хелпер: `server/internal/testdb` (Postgres
   через testcontainers + goose Go-API с `-table goose_db_version_<module>` на
   модуль, повторяет Makefle/compose и ловит silent-skip одинаково названных
   миграций).
...
- **Юнит-тесты чистой логики**: маппинг `connect.Code` → HTTP, сериализация,
  fetchers, zustand stores, `contactHref`/`formatEventRange`-стиль функции.
- **BFF e2e-тесты** (`route.e2e.test.ts`) — НЕ мокают `tournamentToJson` и
  аналоги; реальный `toJson` по реальному proto-ответу, мок-транспорт
  connect-es через `vi.stubGlobal("fetch", ...)`. Ловят proto3-omitted
  (пустой сидtournament), NaN-enum, round-trip опциональных timestamp'ов
  (see ADR 0010). Суффикс `.e2e.test.ts` для читабельности; в `pnpm test`
  входят по умолчанию (дешёвые, без Docker).
...
| Команда                  | Действие                                  |
| ------------------------ | ----------------------------------------- |
| `make test`              | Сервер: `go test ./...` (без Docker)       |
| `make test-web`          | Клиент: `pnpm test` (unit + e2e-route)    |
| `make test-all`          | Все unit/e2e без Docker (server + web)    |
| `make test-integration`  | Сервер: `go test -tags=integration ./...`  |
|                          | (требует Docker, ADR 0010)                |
