...
/**
 * POST /api/applications/[id]/confirm-payment — подтвердить оплату
 * (секретарь/admin; роль проверяется сервером).
 */
export async function POST(_req: NextRequest, ctx: RouteContext): Promise<NextResponse> {
  ...
  try {
    const res = await applicationAdminClient.confirmPayment(
      { applicationId: id },
