...
vi.mock("@/lib/grpc/client", () => ({
  stageAdminClient: { controlArenaTimer: vi.fn() },
}));
...
import { TimerCommandKind } from "@/gen/hema/v1/stage_pb";
...
describe("app/api/arenas/[id]/timer route", () => {
  ...
  it("returns 401 when no access token", async () => {
    ...
    expect(stageAdminClient.controlArenaTimer).not.toHaveBeenCalled();
  });
  ...
  it("relays START/PAUSE/RESET without amountSeconds (defaults to 0)", async () => {
    vi.mocked(getAccessToken).mockResolvedValue("token");
    vi.mocked(stageAdminClient.controlArenaTimer).mockResolvedValue({ snapshot: {} } as never);

    await POST(postReq({ kind: "PAUSE" }), { params: Promise.resolve({ id: "a1" }) });

    expect(stageAdminClient.controlArenaTimer).toHaveBeenCalledWith(
  ...
  it("maps ConnectError NotFound → 404", async () => {
    vi.mocked(getAccessToken).mockResolvedValue("token");
    vi.mocked(stageAdminClient.controlArenaTimer).mockRejectedValue(
