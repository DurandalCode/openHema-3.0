...
vi.mock("@/lib/grpc/client", () => ({
  stageAdminClient: { seatPoolOnArena: vi.fn() },
}));
...
describe("app/api/pools/[poolId]/seat route", () => {
  ...
  it("returns 401 when no access token", async () => {
    ...
    expect(stageAdminClient.seatPoolOnArena).not.toHaveBeenCalled();
  });
  ...
  it("seats the pool and returns layout JSON on ok", async () => {
    vi.mocked(getAccessToken).mockResolvedValue("token");
    vi.mocked(stageAdminClient.seatPoolOnArena).mockResolvedValue({
    ...
    expect(stageAdminClient.seatPoolOnArena).toHaveBeenCalledWith(
  ...
  it("maps ConnectError FailedPrecondition (not ready/already seated/arena busy) → 409", async () => {
    vi.mocked(getAccessToken).mockResolvedValue("token");
    vi.mocked(stageAdminClient.seatPoolOnArena).mockRejectedValue(
