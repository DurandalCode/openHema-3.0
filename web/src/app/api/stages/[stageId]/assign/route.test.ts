...
function req(body: unknown) {
  return new NextRequest("http://localhost/api/stages/s1/assign", {
    method: "POST",
...
describe("app/api/stages/[stageId]/assign route", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("returns 400 when fighterId is missing", async () => {
    ...
    const res = await POST(req({ poolId: "p1" }), { params: Promise.resolve({ stageId: "s1" }) });
  ...
  it("assigns fighter and returns layout JSON on ok", async () => {
    ...
    expect(stageAdminClient.assignFighter).toHaveBeenCalledWith(
      { stageId: "s1", fighterId: "f1", poolId: "p1" },
