...
/**
 * POST /api/stages/[stageId]/undo — «Отменить»: откатить последнее
 * mutating-действие (автораспределение, удаление пула, посев сетки —
 * FR-7a).
 */
export async function POST(_req: NextRequest, ctx: RouteContext): Promise<NextResponse> {
  ...
  try {
    const res = await stageAdminClient.undo(
      { stageId },
