 ...
 * осознанно не заполняется. `stageId` — этап, которому принадлежит контейнер
 * (спека 0018, FR-11a). Опционален: не все существующие фикстуры/потребители
 * ещё заполняют его, а строгая обязательность сломала бы их без причины —
 * поле аддитивное, читатели (сетка) не обязаны требовать presence.
 */
export type Pool = {
  id: string;
  nominationId: string;
