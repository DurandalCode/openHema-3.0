...
// setup поднимает реальный Connect-хендлер StageAdminService с fake-репо и
...
func setup(t *testing.T) (hemav1connect.StageAdminServiceClient, *testutil.FakeRepo, *testutil.FakeActiveFightersProvider) {
	t.Helper()
	admin, _, repo, fighters, _, _, _, _ := setupFull(t)
	return admin, repo, fighters
}

// setupFull — как setup, но дополнительно монтирует StagePublicService (без
...
func setupFull(t *testing.T) (
	hemav1connect.StageAdminServiceClient,
...
func TestListPublicPools_E2E_ReadyShowsPools(t *testing.T) {
	...
	// Публичный запрос — без Authorization: StagePublicService смонтирован
...
// StagePublicService (без RequireAdmin, как ListPublicPools) — публичный
...
// для этого трека scope (только server/modules/stage/**). Флагируется
