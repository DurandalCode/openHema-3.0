...
type Repository interface {
	...
	// SeededPresetKeys — ВСЕ ключи каталога, когда-либо заводившиеся в этой
	// инсталляции (журнал, FR-7), независимо от того, жив ли ещё
	// результирующий пресет: единожды попробовали — не трогаем при
	// автоматическом заведении на старте, даже если пресет потом удалили.
	SeededPresetKeys(ctx context.Context) ([]string, error)
	// LiveSeededPresetKeys — поднабор SeededPresetKeys, чей заведённый
	// пресет ещё существует в библиотеке (FR-10): переименование пресет не
	// убивает (id тот же), поэтому переименованная встроенная запись
	// остаётся «живой» — явное восстановление обязано оставить её
	// нетронутой и не заводить дубликат под освободившимся старым именем.
	// Удаление пресета обнуляет связь (ON DELETE SET NULL, миграция 00006)
	// — такой ключ выпадает отсюда и восстановление заводит его заново.
	LiveSeededPresetKeys(ctx context.Context) ([]string, error)
	// MarkPresetSeeded отмечает попытку заведения ключа (идемпотентно,
	// upsert): presetID — id заведённого пресета, либо "" (NULL), если
	// заведение было пропущено по занятому имени (FR-9).
	MarkPresetSeeded(ctx context.Context, key, presetID string) error
...
// табло (полноэкранный зрительский экран, участвует в нумерации) или панель
// управления ареной (this_ordinal всегда 0). Источником таймера становится
// первое табло, а если табло в комнате нет — первая панель (фоллбэк,
// уточнение ADR 0013 §1/§4; см. arenaRoom.source).
...
// источником таймера (табло ordinal 1, либо первая панель, если табло нет).
// Пара (ScoreboardCount == 0 && ThisIsSource) — код «табло не подключено,
// таймер идёт на этом экране», по которому консоль рисует предупреждение.
// SidesSwapped — эфемерный swap синий/красный (FR-6), не персистится за
// пределами живой комнаты.
