...
type Pool struct {
	...
	// Standings — итоговая таблица пула (спека 0016): пусто, если у пула нет
	// ни одного завершённого боя (FR-7) либо на пути чтения, где таблица
	// осознанно не заполняется (арена/табло, см. service).
	Standings []Standing
}
